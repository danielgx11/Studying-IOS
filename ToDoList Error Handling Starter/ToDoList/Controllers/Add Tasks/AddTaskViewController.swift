...
class AddTaskViewController: UIViewController {
    ...
    @IBAction func addTaskDidTouch(_ sender: Any) {
        ...
        do {
            taskName = try validation.validate(text: taskNameTextField.text, minLenght: 4, maxLenght: 9)
        } catch ValidationError.isEmpty {
            reportError(title: "Task Name Empty", message: "Task name field is required")
            return
        } catch ValidationError.isShort {
            reportError(title: "Task Name Short", message: "Please enter a task name that's between 4 and 9 characters long")
            return
        } catch ValidationError.isLong {
            reportError(title: "Task Name Long", message: "Please enter a task name that's between 4 and 9 characters long")
            return
        } catch {
